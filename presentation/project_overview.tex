...
\item location $\tau = \mu - \theta \cdot \kappa$
\end{itemize}
\end{frame}
...
\begin{itemize}
 \item Prior: $\beta \sim \text{Gamma}(a_0, b_0)$   
 \item Likelihood: $x_i | \beta \sim \text{Gamma}(\alpha, \beta)$
 \item Analytical posterior (with $n$ observed data points): $\beta | x_{1:n}\sim \text{Gamma}(a_0+n\alpha, b_0+\sum_{i=1}^n x_i)$
\end{itemize}
\end{frame}
...
\item update sufficent statistics
	\begin{itemize}
		...
		\item $S \gets S+x_t$
	\end{itemize}
\item Posterior becomes $\beta|x_{1:n} \sim \text{Gamma}(a_0+n\alpha, b_0 +S)$
\end{itemize}
\end{frame}
